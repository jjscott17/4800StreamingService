...
const Watch = () => {
  ...
  useEffect(() => {
    fetch('https://4800api.sdvxindex.com/api/movies')
      .then(res => res.json())
      .then(data => {
        const foundMovie = data.find(movie => movie.title === decodeURIComponent(movieTitle));
        if (foundMovie && foundMovie.stream_url) {
          setVideoUrl(foundMovie.stream_url);
        } else {
          console.warn('Movie not found or missing videoUrl');
        }
      })
